    ...
    if ((cds->number_of_memory_reference % cds->LFU_Decay_Interval) == 0)
        {
            ...
            for (i = 0; i < cds->number_of_cache_entries; i+=4)
                {
                    cds->c[i].replacement_data = cds->c[i].replacement_data>>1;
                    cds->c[i+1].replacement_data = cds->c[i+1].replacement_data>>1;
                    cds->c[i+2].replacement_data = cds->c[i+2].replacement_data>>1;
                    cds->c[i+3].replacement_data = cds->c[i+3].replacement_data>>1;
                }
        }
}
...
int Replacement_policy(CDS *cds, int first_index, int set_size)
{
    ...
    switch (cds->replacement_policy)
        ...
        case CRP_LFU:  /* replacement data is the number of uses, so choose the smallest */
        {
...
memory_address Base_Cache_Address(CDS *cds, memory_address a)
{
    ...
    memory_address low_order_mask = mask_of(cds->number_of_low_order_bits);
...
void Simulate_Reference_to_Cache_Line(CDS *cds, memory_reference *reference)
{
    ...
    memory_address sets_bits_mask = mask_of(cds->sets_bits);
    int cache_set_index = (cache_address >> cds->number_of_low_order_bits) & sets_bits_mask;
    ...
    int i, dirty;
    dirty = reference->type == MAT_STORE;
    if (cds->number_of_ways == 1024){
        for (i = 0; i < cds->number_of_ways; i+=4)
        {
            cache_line current = cds->c[cache_entry_index+i];
            if ((cache_address == current.tag))
                {
                    if (!current.valid)
                        break;
                    /* found it -- record cache hit and exit */
                    //if (debug) fprintf(debug_file, "%s: Found address 0x%08X in cache line %d\n", cds->name, reference->address, cache_entry_index+i);
                    cds->number_cache_hits += 1;

                    /* update reference specific info */
                    if (dirty) 
                        cds->c[cache_entry_index+i].dirty = dirty;
                    Update_Replacement_Policy_Data(cds, cache_entry_index, i);
                    return;
                }
            current = cds->c[cache_entry_index+i+1];
            if ((cache_address == current.tag))
                {
                    if (!current.valid)
                        break;
                    /* found it -- record cache hit and exit */
                    //if (debug) fprintf(debug_file, "%s: Found address 0x%08X in cache line %d\n", cds->name, reference->address, cache_entry_index+i+1);
                    cds->number_cache_hits += 1;

                    /* update reference specific info */
                    if (dirty) 
                        cds->c[cache_entry_index+i+1].dirty = dirty;
                    Update_Replacement_Policy_Data(cds, cache_entry_index, i+1);
                    return;
                }
            current = cds->c[cache_entry_index+i+2];
            if ((cache_address == current.tag))
                {
                    if (!current.valid)
                        break;
                    /* found it -- record cache hit and exit */
                    //if (debug) fprintf(debug_file, "%s: Found address 0x%08X in cache line %d\n", cds->name, reference->address, cache_entry_index+i+2);
                    cds->number_cache_hits += 1;

                    /* update reference specific info */
                    if (dirty) 
                        cds->c[cache_entry_index+i+2].dirty = dirty;
                    Update_Replacement_Policy_Data(cds, cache_entry_index, i+2);
                    return;
                }
            current = cds->c[cache_entry_index+i+3];
            if ((cache_address == current.tag))
                {
                    if (!current.valid)
                        break;
                    /* found it -- record cache hit and exit */
                    //if (debug) fprintf(debug_file, "%s: Found address 0x%08X in cache line %d\n", cds->name, reference->address, cache_entry_index+i+3);
                    cds->number_cache_hits += 1;

                    /* update reference specific info */
                    if (dirty) 
                        cds->c[cache_entry_index+i+3].dirty = dirty;
                    Update_Replacement_Policy_Data(cds, cache_entry_index, i+3);
                    return;
                }
        }
    }
